 ...
 u4 fi(u4 fi_in, u4 fi_key)
{
 ...
 u4  fo(u4  *ek, u4  fo_in, unsigned char k)
{
  u4  t0, t1;
  t0 = (fo_in >> 16);
  t1 = fo_in & 0xFFFF;
 ...
 u4  fl(u4  *ek, u4  fl_in, unsigned char k)
{
  u4  d0, d1;
  unsigned char t;
 ...
 u4  flinv(u4  *ek, u4  fl_in, unsigned char k)
{
  u4  d0, d1;
  unsigned char t;
...
static void bcopy_u4_byte(u4 k, unsigned char * b) { 
...
typedef struct mky
{
	...
	u4 encrypt[MISTY1_KEYSIZE];
	//u4 decrypt[MISTY1_KEYSIZE];
...
void mistyinitialize(u4 *keypntr,int index)
{
	misty1_keyinit(mistkeys[index].encrypt,keypntr);

}


void mistyencipher(u4 * bu,int index)
{

	 misty1_encrypt_block((u4 *) &mistkeys[index], bu, bu);

}

void mistydecipher(u4 * bu,int index)
{
  misty1_decrypt_block((u4 *) &mistkeys[index], bu, bu);
}
...
*/
    u4  ek[MISTY1_KEYSIZE] = {0};
    u4 fl_input = {0xABCD0000};
    u4 temp = 0;

    memset(ek, 0x00,  MISTY1_KEYSIZE * sizeof(u4));

    printf("FL:\tin:\t%8.8X\n", fl_input);
    temp = fl(ek, fl_input, 0);
    printf("FL:\tout:\t%8.8X\n", temp);
    ...
    if (!memcmp(c,Ciphertext,4 * sizeof(u4))) {
        ...
        printf("Encryption failed[0x%08x 0x%08x 0x%08x 0x%08x]\n",
               c[0],c[1],c[2],c[3]);
        return (1);
    }
    ...
    if (!memcmp(c,Plaintext,4 * sizeof(u4))) {
        ...
        printf("Decryption failed[0x%08x 0x%08x 0x%08x 0x%08x]\n",
               c[0],c[1],c[2],c[3]);
        return(1);
    }
